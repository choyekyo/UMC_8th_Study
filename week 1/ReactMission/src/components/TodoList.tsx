import { TTodo } from "../assets/types/todo";
...
const TodoList = ({title, todos, buttonLabel, buttonColor, onClick}:TodoListProps) => {
    ...
    <div className='render-container_section'>
        ...
        {todos?.map((todo) => (
            ...
            <button onClick = {():void => onClick(todo)} style={{backgroundColor: buttonColor,}} className='render-container_item-button'>{buttonLabel}</button>
            </li>
        ))}
        </ul>
    </div>
